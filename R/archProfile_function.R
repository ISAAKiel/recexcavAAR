...
#Die Datei muss zwei Spalten haben, zum einen die mit der Gruppierung für die Profile (z.B. Profilnummer), und (optional) eine
...
profile_id <- 1
while(profile_id<length(colnames(fotogram_pts@data)) && colnames(fotogram_pts@data)[profile_id] != profile_col)
{
...
#Spalte mit dem NAmen raussuchen und den Spaltenindex merken
view_id <- 1
while(view_id<length(colnames(fotogram_pts@data)) && colnames(fotogram_pts@data)[view_id] != view_col)
{
  view_id <- view_id+1
}


#Die Punkte mssen umden Mittelpunkt rotiert werden, sodass sie parallel zur x achse liegen
...
#Nas raus
coord <- na.omit(coord)
#Alle Namen der Profile ermitteln, da sich daraus die Anzahl der Schleifenduchläufe bestimmt
prnames <- levels(as.factor(coord$pr))
#Eine Tabelle, die für den Export da ist, mit der gleichen Datenlänge wie die importierte
#Da dieser df am ende immer wieder neue daten angehängt bekommt und das rbind mit df blöd ist, wird hier eine Kopie der orginaltabelle überschrieben
...
while (i <= length(prnames)){
  ...
  #Je gerader die Profile, desto genauer. Man könnte auch mit Matritzen arbeiten, ich denke aber, dass das übertrieben ist.

  #Um die Abweichungen in des ungeraden Profils etwas auszugleichen wird eine Augleichsgerade zwischen die Punkte geschrieben
  ...
  #Nun muss unterschieden werden, ob das Profil im oder gegen der Uhrzeigersinn gedreht werden muss.
  #Das wird durch 2 Parameter bestimmt, Steigung und view. Es gibt vier Fälle (Sonderfälle sind m = 0)
  ...
  if (slope < 0 && coord_proc$view[1] == "N" || slope < 0 && coord_proc$view[1] == "E"){
    slope_deg <- ((atan(slope)*180)/pi)
  } else if (slope < 0 && coord_proc$view[1] == "S" || slope < 0 && coord_proc$view[1] == "W"){
    slope_deg <- 180 + ((atan(slope)*180)/pi)
  } else if (slope > 0 && coord_proc$view[1] == "S" || slope > 0 && coord_proc$view[1] == "E") {
    slope_deg <- 360 - ((atan(slope)*180)/pi)
  } else if (slope > 0 && coord$view[i] == "N" || slope > 0 && coord$view[i] == "W") {
  ...
  center_y <- predict(fm, data.frame(xw = c(center_x)), se.fit = TRUE)$fit
  ...
  #Für jeden Punkt des Profils mittels translation und rotation den neuen Punkt bestimmen
  for (z in 1:nrow(coord_proc)){
    coord_trans[z,] <- c(
      center_x + (coord_proc$x[z] - center_x) * cos(slope_deg / 180 * pi) - sin(slope_deg / 180 * pi) * (coord_proc$y[z] - center_y),
      center_y + (coord_proc$x[z] - center_x) * sin(slope_deg / 180 * pi) + (coord_proc$y[z] - center_y) * cos(slope_deg / 180 * pi),(coord_proc$z[z]+center_y-mean(coord_proc$z)),coord_proc$pr[z])
...
export <- SpatialPointsDataFrame(coords=coord_export[,c(1,3)], data = coord_export, proj4string = (fotogram_pts@proj4string))
return(export)
}