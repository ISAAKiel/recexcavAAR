...
Hier eine kleine Erklärung zur Fundktionsweise:

##Ziel
Ziel ist es die Fotogrammetriepunkte eines Profils, die mit x, y und z Koordinaten beschrieben sind so zu rotieren und zu klappen, dass man im GIS in der 2D Aufsicht diese georeferenzieren kann. Im Georeferencer gibt es den Transformationstyp projektiv, der eine Entzerrung ermöglicht.

##Durchführung
In der Ansicht im GIS würden die Messpunkte zunächst so aussehen.
Das Profil ist in diesem Fall um 20° nach hinten gekippt, d.h. die Punkte in SO liegen weiter unten.
...
#Fuer das Beidpiel nur ein Profil
...
*Hier könnte überlegt werden, ob man den Schritt auch optional macht, d.h. die Profile nachher wieder zurückdreht, so würde es die Möglichkeit geben, dass die Profile später an der alten Stelle liegen*
...
Da die Spalten nicht immer gleich sind - wohingegen die Koordinatenspalten in einem SDF fest definier sind ;-) - können die Beschriftungen als Parameter übergeben werden und die Spalten werden herausgesucht.
...
Dann wird die Drehung des Profils in die Horizontale eingeleitet. Dazu wird die Regressionsgerade durch die Punkte bestimmt.
...
*Es stellt sich die Frage, ob es sinnvoll ist die inneren Punkte herauszunehmen, da diese evt nicht mittig liegen und dann zu starken Einfluss bekommen*
...
Der nächste Schritt beinhaltet das Drehen (Ansicht von Nord).
...
Es ist also gekippt. Die Entzerrung geht so nicht, da die Blickwinkel immer orthogonal zu den Achsen ist.
Also müssen die oben durchgeführten Schritte für die Rotation um die z-Achse wiederholt werden.
Hier gab es das Problem mit den zu großen Zahlen.
...
Und wieder drehen, diemal um die X-Achse
...
Und dann die umwandlung in einen sdf, wobei x und y vertauscht sind, sodass ich die Ansicht auf das Profil bekomme:
...
- Prüfen warum bei langen Profilen nicht genau
  - Rotation um dritte Achse nötig?
  - Fehler durch innenliegende Punkte
