...
We can put this two or even more data.frames with observations into a list and feed it to `recexcavAAR::kriglist`. This function serves as an interface to `recexcavAAR::kriging`. We'll get a list list of data.frames with surface estimations for the two input layers. 
...
The result of `recexcavAAR::kriging` is in a tall format -- we have to transform it. For this purpose we use `recexcavAAR::spatialwide2`.

```{r}
surf1 <- spatialwide2(maps[[1]]$x, maps[[1]]$y, maps[[1]]$pred, 3)
surf2 <- spatialwide2(maps[[2]]$x, maps[[2]]$y, maps[[2]]$pred, 3)
```
...
```{r fig.width=7, fig.height=5}
vis <- vis %>% 
  add_trace(x = surf1$x, y = surf1$y, z = surf1$z, type = "surface", showscale = FALSE
  ) %>%
  add_trace(x = surf2$x, y = surf2$y, z = surf2$z, type = "surface", showscale = FALSE
  ) 
...
Now we can fill the shape with an equidistant three dimensional point raster using `recexcavAAR::fillhexa` and take a look at it.
