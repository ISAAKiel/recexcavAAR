...
```
As shown we only interchanged the Y-value of the first and second coordinate. After running the *cootrans*-function for the second time ...
```{r}
absolute_data <- cootrans(coord, c(3,4,1,2), measured_points, c(1,2))
```
... everything seems fine. No warning messages pop up.  
